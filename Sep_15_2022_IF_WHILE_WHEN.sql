...
SELECT 客戶編號,產品編號,數量
FROM
	(SELECT 客戶編號,產品編號,數量,RANK() OVER(PARTITION BY 產品編號 ORDER BY 數量 DESC) AS 序號
     FROM
		 (SELECT B.客戶編號,產品編號, SUM(數量) AS 數量
		  FROM 訂貨明細 AS A OUTER APPLY (SELECT 客戶編號 FROM 訂貨主檔 WHERE 訂單號碼=A.訂單號碼) AS B 
		  GROUP BY B.客戶編號,產品編號) AS C) AS D
WHERE 序號 BETWEEN 1 AND 5
ORDER BY 產品編號,數量 DESC
